...
for entry in data:
    corpus += re.sub(r'[^a-zA-Z\d\s]', "", entry['text']) + " "
    ...
    for entry in data:
        ...
        for word in words_count:
            ...
            if document_length != 0 :
                word_probability = (0.5 * (occurances_document/document_length)) + ((1-0.5) * (occurences_corpus/corpus_length))
...
with open('task3b_top_documents_jm.csv', 'w', newline='') as csv_file:
    writer = csv.writer(csv_file)
    for result in top_results:
        writer.writerow(result)

# as json for easy future loading
with open('task3b_top_documents_jm.json', 'w') as json_file:
