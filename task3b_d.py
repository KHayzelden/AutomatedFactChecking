...
# applying Dirichlet smoothing with the following formula
# λ(c(w,d)/|d|) + (1-λ)(c(w,a)/|a|) where a is all documents and λ is the average document length
# so this translates to :
# N/(N+u)*(c(w,d)/|d|) + (u/N+u)(c(w,a)/|a|)
# where u is a default value and N is the document length
# u is typically the average length of all documents
...
#------------------------------------------------------------------------------#
#                   GENERATING CORPUS FOR SMOOTHING LATER
#------------------------------------------------------------------------------#
...
for entry in data:
    temp_corpus = re.sub(r'[^a-zA-Z\d\s]', "", entry['text']) + " "
    lengths.append(len(temp_corpus.split()))
    corpus += temp_corpus

corpus_length = len(corpus.split())
average_length = float(sum(lengths)) / float(len(lengths)) # used as "u" in smoothing
...
#------------------------------------------------------------------------------#
    ...
    # counting appearances of each word in the corpus for the smoothing later

    counter = 0

    for word in filtered_words:
        corpus_count = corpus.lower().count(word)
        counts[word] = corpus_count
        counter += 1
        print(counter, "/", len(filtered_words))

    # applying jelinek-mercier smoothing with the following formula
    # λ(c(w,d)/|d|) + (1-λ)(c(w,a)/|a|) where a is all documents
    # setting lambda to a meta 0.5
    ...
    for entry in data:
        ...
        document_length = len(temp_text.split())
        
        # counts the occurance of each word
        for word in words_count:
            
            word_probability = 0
            occurances_document = temp_text.lower().count(word)
            occurences_corpus = counts[word]
            
            if document_length != 0 :
                # N / ( N + u ) to give more weight to longer documents
                smoothing_document = document_length/ float(document_length * average_length)
            
                # u / ( N + u )
                smoothing_corpus = average_length / float(document_length * average_length)
            
                word_probability = (smoothing_document * (occurances_document/document_length)) + (smoothing_corpus * (occurences_corpus/corpus_length))
...
with open('task3b_top_documents_d.csv', 'w', newline='') as csv_file:
    writer = csv.writer(csv_file)
    for result in top_results:
        writer.writerow(result)

# as json for easy future loading
with open('task3b_top_documents_d.json', 'w') as json_file:
