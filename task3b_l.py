...
# then applies laplace smoothing to the query likelyhood model
...
#------------------------------------------------------------------------------#
    ...
    for entry in data:
        ...
        # there a few tiny claims that will always rank really high due to their small word
        # count and therefore need to be discredited
        if length < 20:
            length = 100
        ...
        for word in filtered_words:
            ...
            # laplace smoothing where occurances + 1 / the length of the document plus the length of the claim's vocab
            word_probability = float(occurances + 1) / float(length + len(words))
...
with open('task3b_top_documents_l.csv', 'w', newline='') as csv_file:
    writer = csv.writer(csv_file)
    for result in top_results:
        writer.writerow(result)

# as json for easy future loading
with open('task3b_top_documents_l.json', 'w') as json_file:
